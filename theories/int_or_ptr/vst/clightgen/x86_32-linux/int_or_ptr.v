...
Definition f_int_or_ptr__sizeof := {|
  ...
  fn_temps := nil;
  fn_body :=
(Sreturn (Some (Esizeof (talignas 2%N (tptr tvoid)) tuint)))
|}.
...
Definition f_int_or_ptr__alignof := {|
  ...
  fn_temps := nil;
  fn_body :=
(Sreturn (Some (Ealignof (talignas 2%N (tptr tvoid)) tuint)))
|}.
...
Definition f_int_or_ptr__is_int := {|
  ...
  fn_vars := nil;
  fn_temps := ((_zero, tint) :: (_one, tint) :: nil);
  fn_body :=
(Ssequence
  ...
  (Ssequence
    ...
    (Ssequence
      (Sifthenelse (Ebinop Oeq (Etempvar _zero tint)
                     (Ebinop Oand
                       (Ecast (Etempvar _x (talignas 2%N (tptr tvoid))) tint)
                       (Etempvar _one tint) tint) tint)
